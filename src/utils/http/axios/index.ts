...
import { RequestEnum, ResultEnum, ContentTypeEnum, HttpErrorEnum } from '@/enums/httpEnum';
...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    //   createErrorModal({ title: HttpErrorEnum.errorTip, content: timeoutMsg });
    // } else if (options.errorMessageMode === 'message') {
    //   createMessage.error(timeoutMsg);
    // } else if (options.errorMessageMode === 'notification') {
    //   notification.error({
    //     message: HttpErrorEnum.errorTip,
  ...
  responseInterceptorsCatch: (error: any) => {    
    ...
    try {
      if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
        errMessage = HttpErrorEnum.apiTimeoutMessage;
      }
      if (err?.includes('Network Error')) {
        errMessage = HttpErrorEnum.networkExceptionMsg;
      }

      // if (errMessage) {
      //   if (errorMessageMode === 'modal') {
      //     createErrorModal({ title: HttpErrorEnum.errorTip, content: errMessage });
